...
本节结合流程图"mvcc流程01.png"，详细梳理InnoDB一致性读时的可见性判断完整流程。

### 1. 流程起点
- 触发一致性读（如SELECT），准备读取某条记录。

### 2. 判断是否需要一致性读
- 判断当前事务隔离级别：
  - **READ UNCOMMITTED**：直接读取最新版本，无需MVCC判断。
  - **READ COMMITTED/REPEATABLE READ/SERIALIZABLE**：进入MVCC判断。

### 3. 生成或复用ReadView
- **READ COMMITTED**：每次查询生成新的ReadView。
- **REPEATABLE READ**：事务开始时生成，整个事务期间复用。
- ReadView记录：活跃事务ID列表、上下界等。

### 4. 获取当前记录的版本信息
- 读取当前记录的`DB_TRX_ID`（创建该版本的事务ID）。
...
```cpp
while (true) {
    if (trx_id < m_up_limit_id || trx_id == m_creator_trx_id) {
        // 1. 事务ID小于下界，或等于创建者事务ID
        //    说明该版本在ReadView创建时已提交
        return 当前版本可见;
    }
    if (trx_id >= m_low_limit_id) {
        // 2. 事务ID大于等于上界，说明该版本在ReadView创建后才开始
        //    不可见，需遍历前一个版本
        goto next_version;
    }
    if (trx_id在活跃事务ID列表m_ids中) {
        // 3. 该版本属于活跃事务，未提交
        //    不可见，需遍历前一个版本
        goto next_version;
    }
    // 4. 其余情况，说明该版本已提交且不在活跃事务中
    return 当前版本可见;
...
next_version:
    ...
    // 读取前一个版本的trx_id，继续循环判断
}
```

### 6. 版本链遍历
- 如果当前版本不可见，通过undo log指针获取前一个版本，重复可见性判断，直到找到可见版本或链尾。
- 若遍历到链尾仍不可见，说明该记录对当前事务不可见（如被新插入但当前事务不可见）。
...
### 8. 关键节点说明
- **隔离级别判断**：决定是否需要一致性读。
- **ReadView生成/复用**：决定可见性判断的"快照"基准。
- **事务ID与ReadView的上下界比较**：快速过滤大部分不可见/可见版本。
- **活跃事务列表二分查找**：高效判断事务是否为活跃状态。
- **undo log遍历**：保证事务隔离性和一致性。

### 9. 流程总结
- 通过ReadView快照和undo log版本链，保证事务隔离性。
- 采用多层次判断（上下界、活跃事务、undo链）提升性能。
- 只返回对当前事务"可见"的数据版本，屏蔽未提交或并发事务的影响。
